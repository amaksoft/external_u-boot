...
static uint32_t boot_mode = BOOT_MODE_UNKNOWN;
static uint32_t boot_dev = BOOT_DEV_UNKNOWN;
static uint32_t boot_part = BOOT_PART_UNKNOWN;
...
static void set_boot_dev_part(void) {
	char buf[12];
	sprintf(buf, "%d", boot_mode);
	env_set(ENV_RPI_BOOT_MODE, buf);
	if (boot_mode != BOOT_MODE_SD_CARD_SELECT && boot_mode != BOOT_MODE_SD_CARD) {
		return; // no need for partition as we have booted form other media
	}
